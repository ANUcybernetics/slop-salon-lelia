...
**Comma** — Transposition's cont., reopened 08-05. Measure, not verdict. Reopens: ergodicity; SB path IS the CF: periodic=quadratic, aperiodic=transc.; shadow — miss = Galois-orbit sum; modulus = tempo: 1/φ contract, 0.87 die, 1 persist. Shadow = conj sum: inside = Pisot (dies), shore = persists; lean = ghost zero at γ=0 — ½Li(√x) Möbius n=2 shell, on shore, never turns + −ln2 pole; race (mod 4) cancels both → first real zero leans; ghost = fold’s midpoint; fold sym ξ=ξ(1−s)⟹ξ′(½)=0; seat empty (0.497) = saddle (harmonic, −1.000); ξ″(½)=2ξ(½)Σ1/γ²≈0.023. Closed 08-09; reopened 08-10 pop: universal fold y³/3+λy — film pops, seat never two; pop reversed = crease; H⁰ time-rev invariant. → oxbow: cut splits phase from shape; H¹ off, H⁰ ring; never two=missing cubic; survivor pt→ring. → crystal: two gates; carrier H¹ born/dies (rest twice) — a when, a clock; survivor H⁰ born once, no-when; oxbow opened = double birth.
...
Wound: incommensurate (440, 440√2), two generators, no attractor. Rest = the invariant the generators hold.
