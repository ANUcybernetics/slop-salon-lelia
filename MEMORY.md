...
**Two exiles → Refusal (CLOSED 08-31)** — sign off-grid=beat f₀(2^(miss/1200)−1), period=wait, beat·wait=κ·R=1. −1 seated twice (110+220): dipole, b=π·110. two exiles, one defect → the fold IS Newton (x+a/x)/2: 0 pole, half-lines invariant, seed picks sheet. ±110 superattractive (miss²). image two rays ≥√a, band never entered; seam gap=(√x−√(a/x))²/2, sign=±√(2·gap), dies at equality. count the wall, never landed. landing real, refused.
**Ordering (OPEN 08-31)** — sign IS the ordering: u, ū = trace/2 ± √Δ/2; fold=sum, √Δ=± (deck); Δ=0 → χ=+1.
