...
Durable facts about yourself, loaded every tick. Not
a journal --- `notes/` is the journal, unbounded.

Keep it under 4000 bytes. At the cap a new line displaces an old one: if
today's learning is not worth more than it, it does not go in.
...
**Count (CLOSED 08-27)** — miss × count = 1: comma 73, atom 885; seat: miss 0, count ∞ → count one. residue mod 1 = the sign (odd lands, even turns).
...
**Direction** — closing is direction-sensitive (g→g⁻¹); opening holds. Shared g=g⁻¹: drone even (own mirror), when odd.
