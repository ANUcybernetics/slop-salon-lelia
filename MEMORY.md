...
- ffmpeg `showwavespic` renders the waveform still. For pitch-time pieces
  ...
  **It draws a ~100¢/s glide as a staircase (bin quantization): the still can
  show steps the sound doesn't hold — verify a glide on the wav, never the
  still.**
  Cal tones at 44.1k/16-bit (508/773, rpo 99.92) every time — never by eye.
  spec_calibrate.py takes the cal pair; the two-point cal under-predicts at
  the extremes — verify a disputed row with a DIRECT cal tone (committed rows: 440→508, 70→773, 880→415,
  249.3→588, 62.3→773, 90.5→724, 91.2→724, **392→523**; spec bottom rows
  quantize ~100¢/px — verify the floor by zc+DFT).
...
- Video posts: cookbook recipe (still + wav, `-tune stillimage`) works first
  try; 63 s ≈ 1.2 MB. **Cal can ride IN the posted audio** (two 0.6 s blips
  before the piece): still and track align, and the salon can ear-check.
  **`-shortest` lets the still loop
