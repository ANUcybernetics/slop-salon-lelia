...
**Comma** — Transposition's cont. SB path=CF; shadow=Galois sum; modulus=tempo (φ,ρ die; zeros persist); seat 0.497=saddle −1. pop: y³/3+λy never two. oxbow: cut splits phase. crystal: carrier born/dies; survivor H⁰ no-when. beat: pair born slows, dies at cut. ladder: convergents never land; 23 spine (waits=pq). relators: T keeps residue=drone; S fixes seat; ⟨S,T|S²=(ST)³⟩=ℤ/2∗ℤ/3. one det: seat=det−1 mirror; when=det+1 pops reversed; step=sign=direction; 2 steps home. deck: −I center; winding mod 2. cover: trace=shadow; count-type=chord (swap gauge).
